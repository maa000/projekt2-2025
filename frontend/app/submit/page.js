        ...
        <main className="bg-violet-100 min-h-screen px-4 py-10">
            <div className="bg-white max-w-4xl mx-auto p-8 rounded-xl shadow-md">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label className="block font-semibold mb-1">Étel neve</label>
                        <input
                            value={recipeName}
                    ...
                    <div>
                        <label className="block font-semibold mb-1">Konyha típusa</label>
                        <input
                            value={cuisine}
                    ...
                    <div>
                        <label className="block font-semibold mb-1">Előkészítés ideje (pl. 00:15:00)</label>
                        <input
                            value={prepTime}
                    ...
                    <div>
                        <label className="block font-semibold mb-1">Főzési idő (pl. 00:30:00)</label>
                        <input
                            value={cookTime}
                ...
                {/* Leírás */}
                <div className="mb-6">
                    <label className="block font-semibold mb-1">Leírás</label>
                    <textarea
                        value={description}
                        ...
                        {ingredients.map((item, index) => (
                            <div key={index} className="flex flex-wrap gap-2">
                                <select
                                    ...
                                    className="w-1/3 min-w-[120px] border px-3 py-2 rounded bg-gray-100"
                                >
                                ...
                                <input
                                    ...
                                    className="w-20 border px-3 py-2 rounded bg-gray-100 text-center"
                                />
                                <select
                                    value={item.unit}
                                    onChange={(e) => handleIngredientChange(index, "unit", e.target.value)}
                                    className="w-24 border px-3 py-2 rounded bg-gray-100 text-center"
                                >
                                    <option value="">Egység</option>
                                    {measurementsList.map(measure => (
                                        <option key={measure.id} value={measure.name}>{measure.name}</option>
                                    ))}
                                </select>
                ...
                <div className="text-center">
                    <button className="bg-purple-600 hover:bg-purple-700 text-white px-8 py-3 rounded-md text-lg font-semibold shadow-md transition">
                        Recept beküldése
                    </button>
                </div>
            </div>
